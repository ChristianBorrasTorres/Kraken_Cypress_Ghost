...
describe('Test to edit Tag adding Note with more than the max of characters (500)', () => {
    ...
    it('7 Clear Tag Note and try to save Tag', () => {
    ...
    it('9 Try to edit tag again', () => {
        pageObject.getTagList().contains(tagName).click();
        cy.wait(2000);
        cy.get('textarea[id="tag-description"]').type(faker.random.word());
        cy.wait(1000);
    })

    it('10 Try to save changes', () => {
        cy.get('button[class="gh-btn gh-btn-primary gh-btn-icon ember-view"]').click();
        cy.wait(2000);
        cy.screenshot();
    })
